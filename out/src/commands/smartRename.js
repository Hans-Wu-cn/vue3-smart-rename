...
exports.smartRename = void 0;
const vscode = __importStar(require("vscode"));
const symbolAnalyzer_1 = require("../utils/symbolAnalyzer");
async function smartRename() {
    try {
        ...
        let references = await vscode.commands.executeCommand('vscode.executeReferenceProvider', document.uri, position) || [];
        // 2. 添加符号本身的定义位置
        ...
        // 4. 分类引用
        let currentFileReferences = (0, symbolAnalyzer_1.getReferencesInCurrentFile)(validReferences, document.uri);
        let otherFileReferences = (0, symbolAnalyzer_1.getReferencesInOtherFiles)(validReferences, document.uri);
        // 如果当前文件引用较少，尝试使用专门的单文件查找函数补充
        if (currentFileReferences.length === 0 || currentFileReferences.length < 2) {
            const currentFileRefs = await (0, symbolAnalyzer_1.findAllReferencesInCurrentFile)(document, oldName);
        ...
        // 4. 检查符号是否被导出或有跨文件引用
        const isExported = await (0, symbolAnalyzer_1.isSymbolExported)(document, position);
        // 5. 智能选择更新范围
        ...
        if (isExported || otherFileReferences.length > 0) {
            ...
            if (workspaceFolder) {
                ...
                if (componentName) {
                    const transmissionReferences = await (0, symbolAnalyzer_1.analyzePropsTransmission)(componentName, oldName, workspaceFolder);
                    // 合并透传引用到总引用列表
                ...
                currentFileReferences = (0, symbolAnalyzer_1.getReferencesInCurrentFile)(validReferences, document.uri);
                otherFileReferences = (0, symbolAnalyzer_1.getReferencesInOtherFiles)(validReferences, document.uri);
            ...
            if (updateAllFiles) {
                ...
                currentFileReferences = await (0, symbolAnalyzer_1.findAllReferencesInCurrentFile)(document, oldName);
 ...
 * 执行全局重命名，处理所有文件中的引用
 */
async function renameAllFiles(references, oldName, newName) {
    // 按文件分组引用
            ...
            for (const ref of sortedReferences) {
                try {
                    ...
                    if (currentText !== oldName && currentText !== oldName.toLowerCase() && currentText !== oldName.toUpperCase()) {
                        ...
                        const lineRange = new vscode.Range(ref.range.start.line, 0, ref.range.start.line, fullLineText.length);
                        // 转义oldName用于正则表达式
...
async function renameInCurrentFile(editor, references, oldName, newName) {
...
//# sourceMappingURL=smartRename.js.map