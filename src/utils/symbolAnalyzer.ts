...
export async function isSymbolExported(document: vscode.TextDocument, position: vscode.Position): Promise<boolean> {
  // 获取符号定义
  ...
  if (!definition || !definition.range) return false;
  ...
  if (fileName.endsWith('.vue')) {
    ...
    if (fullText.includes('defineProps')) {
      ...
      const arrayPropsRegex = new RegExp(
        `definePropss*(s*[sS]*?['"]${symbolName}['"]`,
        'g'
      );
      
      // 对象形式（带引号或无引号）：defineProps({ propName: { type: String } })
      const objectPropsRegex = new RegExp(
        `definePropss*(s*[sS]*?['"]?${symbolName}['"]?s*:`,
        'g'
      );
      
      // 函数形式：defineProps(() => ({ propName: String }))
      const functionPropsRegex = new RegExp(
        `definePropss*(s*(s*)s*=>s*[sS]*?['"]?${symbolName}['"]?s*:`,
        'g'
      );
      ...
      // 更宽松的匹配，确保能匹配到
      const loosePropsRegex = new RegExp(
        `definePropss*[^)]*?\\b${symbolName}\\b`,
        'g'
      );
    ...
    if (fullText.includes('defineEmits')) {
      ...
      const arrayEmitsRegex = new RegExp(
        `defineEmitss*(s*[sS]*?['"]${symbolName}['"]`,
        'g'
      );
      
      // 对象形式：defineEmits({ eventName: (payload) => boolean })
      const objectEmitsRegex = new RegExp(
        `defineEmitss*(s*[sS]*?['"]?${symbolName}['"]?s*:`,
        'g'
      );
      ...
      const looseEmitsRegex = new RegExp(
        `defineEmitss*[^)]*?\\b${symbolName}\\b`,
        'g'
      );
    ...
    if (fullText.includes('defineExpose')) {
      ...
      const exposeRegex = new RegExp(
        `defineExposes*(s*[sS]*?\\b${symbolName}\\b`,
        'g'
      );
    ...
    if (fullText.includes('<script setup>') || fullText.includes('<script setup lang=')) {
      ...
      if (setupScriptMatch) {
        ...
        const topLevelRegex = new RegExp(
          `^(?:const|let|var|function|async\\s+function)\\s+${symbolName}\\b`,
          'm'
        );
  ...
  const exportStatementRegex = new RegExp(
 ...
 * 查找所有使用指定组件的位置
 */
export async function findComponentUsages(componentName: string, workspaceFolder: vscode.WorkspaceFolder): Promise<vscode.Location[]> {
  const results: vscode.Location[] = [];
  
  // 生成所有可能的组件名称变体
  const componentNameVariants = new Set([
    componentName,                          // 原始名称
    pascalToKebab(componentName),           // kebab-case变体
    kebabToPascal(componentName),           // PascalCase变体
    componentName.toLowerCase(),            // 全小写变体
 ...
 */
export async function analyzePropsTransmission(componentName: string, propName: string, workspaceFolder: vscode.WorkspaceFolder): Promise<vscode.Location[]> {
  ...
  const componentNameVariants = new Set<string>();
            ...
            if (contentPart) {
              ...
              for (const { pattern, group } of templatePatterns) {
                ...
                while ((templateMatch = pattern.exec(contentPart)) !== null) {
                  ...
                  if (!matchedText || !matchedText.includes(propName)) continue;
 ...
 * 使用文本搜索查找所有引用
 */
export async function findAllTextReferences(symbolName: string, workspaceFolder: vscode.WorkspaceFolder): Promise<vscode.Location[]> {
  const results: vscode.Location[] = [];
  
  // 查找所有相关文件（使用相对路径模式）
  const files = await vscode.workspace.findFiles(
    new vscode.RelativePattern(workspaceFolder, '**/*.{vue,js,ts,jsx,tsx}'),
    new vscode.RelativePattern(workspaceFolder, '**/{node_modules,dist,build,.git}/**')
  );
  ...
  for (const file of files) {
    try {
      ...
      // 检查位置是否在注释或字符串中的辅助函数
      const isInCommentOrString = (offset: number, text: string): boolean => {
        // 获取当前位置之前的文本
      ...
      for (const { pattern, group, description } of searchPatterns) {
        ...
        while ((match = pattern.exec(text)) !== null) {
          // 确定匹配的实际符号位置和长度
          let symbolStart: number, symbolLength: number;
          ...
          if (group === 0) {
            ...
            if (matchIndexInFull === -1) continue;
 ...
 * 用于单文件内变量和函数的重命名
 */
export async function findAllReferencesInCurrentFile(
...
export function getReferencesInCurrentFile(references: vscode.Location[], currentUri: vscode.Uri): vscode.Location[] {
  return references.filter(ref => ref && ref.uri && ref.range && ref.uri.fsPath === currentUri.fsPath);
}

export function getReferencesInOtherFiles(references: vscode.Location[], currentUri: vscode.Uri): vscode.Location[] {
  return references.filter(ref => ref && ref.uri && ref.range && ref.uri.fsPath !== currentUri.fsPath);
}